...
content = content.replace('  return (\n    <div className="min-h-screen bg-[#f3f4f6] text-zinc-900 font-cairo flex flex-col items-center py-6 px-4 md:px-8">', frontPageCode + '\n  return (\n    <div className="min-h-screen bg-[#f3f4f6] text-zinc-900 font-cairo flex flex-col items-center py-6 px-4 md:px-8">');

// Now insert renderFrontPage() in main print
content = content.replace('{paginatedPages.map((pageRows, pageIdx) => {', '{renderFrontPage(false)}\n            {paginatedPages.map((pageRows, pageIdx) => {');

// And insert renderFrontPage(true) in preview
content = content.replace('{previewPagesToDisplay.map((pageRows, pageIdx) => (', '{renderFrontPage(true)}\n                {previewPagesToDisplay.map((pageRows, pageIdx) => (');

fs.writeFileSync('src/components/DailyLogbook.tsx', content, 'utf8');
console.log('Inserted front page renderers');
