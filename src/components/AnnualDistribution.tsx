...
export const AnnualDistribution: React.FC<AnnualDistributionProps> = ({
  ...
  const handleAddItem = (e: React.FormEvent) => {
    ...
    const itemToAdd: AnnualDistributionItem = {
      ...
      month: newItem.month || 'أكتوبر',
      weekNumber: Number(newItem.weekNumber) || 1,
      weekDates: newItem.weekDates || '',
      midan: newItem.midan || (selectedLevel === '4am' ? 'الإنسان والصحة' : 'الإنسان والمحيط'),
      maqta: newItem.maqta || '',
      mawrid: newItem.mawrid || '',
      learningContent: newItem.learningContent || '',
      allocatedHours: Number(newItem.allocatedHours) || 2,
      notes: newItem.notes || '',
      status: 'pending',
    };
