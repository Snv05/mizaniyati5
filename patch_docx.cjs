...
export const generateLogbookDocx = async (
  ...
  const frontPageChildren: any[] = [
    createParagraph("الجمهورية الجزائرية الديمقراطية الشعبية", true, "000000", 36),
    createParagraph("وزارة التربية الوطنية", true, "000000", 32),
    new Paragraph({ text: "", spacing: { after: 600 } }),
    createParagraph("الدفتر اليومي", true, "000000", 48),
    new Paragraph({ text: "", spacing: { after: 600 } }),
    ...
    new Table({
      ...
      rows: [
        new TableRow({ children: [ createCell("الأستاذ(ة):", true, undefined, 1, 1, 28, AlignmentType.RIGHT), createCell(config.teacherName || '—', true, undefined, 1, 1, 28, AlignmentType.RIGHT) ] }),
        new TableRow({ children: [ createCell("المادة:", true, undefined, 1, 1, 28, AlignmentType.RIGHT), createCell("علوم الطبيعة والحياة", true, undefined, 1, 1, 28, AlignmentType.RIGHT) ] }),
        new TableRow({ children: [ createCell("المتوسطة:", true, undefined, 1, 1, 28, AlignmentType.RIGHT), createCell(config.schoolName || '—', true, undefined, 1, 1, 28, AlignmentType.RIGHT) ] }),
        new TableRow({ children: [ createCell("السنة الدراسية:", true, undefined, 1, 1, 28, AlignmentType.RIGHT), createCell(config.schoolYear || '2025 - 2026', true, undefined, 1, 1, 28, AlignmentType.RIGHT) ] }),
        new TableRow({ children: [ createCell("المستويات المسندة:", true, undefined, 1, 1, 28, AlignmentType.RIGHT), createCell(assignedLevels.join(' ، '), true, undefined, 1, 1, 28, AlignmentType.RIGHT) ] }),
      ]
    }),
    
    new Paragraph({ text: "", spacing: { after: 400 } }),
    createParagraph("جدول استعمال الزمن", true, "000000", 30),
  ...
  if (gridRows.length > 0) {
    ...
    timetableRows.push(new TableRow({
      children: [
        createCell("اليوم \\ التوقيت", true, "EEEEEE", 1, 1, 18),
        ...gridRows.map((r: any) => createCell(r.time, true, "EEEEEE", 1, 1, 16))
      ]
    }));
    ...
    WEEK_DAYS.forEach(day => {
      timetableRows.push(new TableRow({
        children: [
          createCell(day, true, "F5F5F5", 1, 1, 18),
          ...gridRows.map((r: any) => createCell(r.cells[day] || '', true, "FFFFFF", 1, 1, 16))
        ]
      }));
    });
  ...
  if (holidays.length > 0) {
    frontPageChildren.push(new Paragraph({ text: "", spacing: { after: 400 } }));
    frontPageChildren.push(createParagraph("جدول العطل", true, "000000", 30));
    frontPageChildren.push(new Paragraph({ text: "", spacing: { after: 200 } }));
    
    const holidayRows: TableRow[] = [];
    holidayRows.push(new TableRow({
      children: [
        createCell("العطلة", true, "EEEEEE", 1, 1, 20),
        createCell("من", true, "EEEEEE", 1, 1, 20),
        createCell("إلى", true, "EEEEEE", 1, 1, 20),
      ]
    }));
    holidays.forEach((h: any) => {
      holidayRows.push(new TableRow({
        children: [
          createCell(h.label, true, "FFFFFF", 1, 1, 18),
          createCell(h.startDate || h.start || '', true, "FFFFFF", 1, 1, 18),
          createCell(h.endDate || h.end || '', true, "FFFFFF", 1, 1, 18),
        ]
      }));
    });
...
`;
fs.writeFileSync('src/utils/docxExportLogbook.ts', NEW_DOCX, 'utf8');
console.log('Updated docxExportLogbook.ts');
